...
- `Order.fee`, `Product.pricePercentageChange24h` and
  `Product.volumePercentageChange24h` are `Decimal?`, the last three `String`
  monetary fields in the model layer. `Order.fee` is empty on orders that have
  not been billed and the 24h changes are signed, both of which
  `nullableDecimal` already handles.
- A test now fails the build if any model reintroduces a `String` price, size,
  rate, total or balance field, the way `decimal_precision_test.dart` already
  guards against `double` and `num`. The `String` half of that claim had gone
  unchecked, which is how #123 survived three conversion PRs.
